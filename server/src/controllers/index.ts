...
import {
  loginReqValidator,
  patientId,
  removeLoginReqValidator,
  removeSpecialistReqValidator,
  setPatientProfileReqValidator,
  setQueueReqValidator,
  setUpdatePatienteReqValidator,
  specialistByspecialtyReqValidator,
  specialistReqValidator,
  specialtyIdReqValidator,
  specialtyReqValidator,
  updateLoginReqValidator,
  updateSpecialtyReqValidator,
} from "./validator";
...
export const specialistByspecialty = (
  req: Request<{ idSpecialty: string }>,
  res: Response
) => {
  const data = specialistByspecialtyReqValidator(req.params);
  if (data) {
    services
      .specialistByspecialtyDB(data)
      .then((resolve) => {
        res.status(200).json(resolve);
      })
      .catch((error) => {
...
export const updatePatient = (req: Request, res: Response) => {
  const updatePatient = setUpdatePatienteReqValidator(req.body);
  if (updatePatient) {
    services
